...
def sanitize_filename(name):
    ...
    # Strip whitespace and collapse multiple spaces/hyphens
    clean_name = re.sub(r'\s+', ' ', clean_name)
    clean_name = re.sub(r'-+', '-', clean_name)
    return clean_name.strip()

def recursive_search_key(obj, key_to_find):
    """Recursively searches a JSON-like object for a specific key."""
    results = []
...
def recursive_search_keys(obj, keys_list):
    """Recursively searches a JSON-like object for first occurrences of keys in keys_list."""
    # Useful for extracting objects representing episodes or offers
    results = []
    if isinstance(obj, dict):
        # If all keys in keys_list are present in this dictionary
...
def parse_rss_feed(feed_url):
    """Fetches and parses an RSS feed URL, returning show details and all episodes."""
...
def parse_apple_podcast_url(url):
    """Parses an Apple Podcasts show or episode page."""
    # Check if URL contains query parameter i (episode ID)
    ...
    if episode_id:
        # It's an episode page
        # Search for objects with both streamUrl and title (which represents the episode)
        episodes_data = recursive_search_keys(server_data, ["streamUrl", "title"])
        if not episodes_data:
            # Fallback to search any streamUrl
        ...
        # Look for the episode that matches the ID or take the first one
        ep = episodes_data[0]
        ...
        # Parse fields from the episode object
        ...
        if release_date and len(release_date) >= 10:
            formatted_date = release_date[:10]  # YYYY-MM-DD
        else:
        ...
        # It's a show page
        # Locate the feedUrl from the JSON
        feed_urls = recursive_search_key(server_data, "feedUrl")
        if not feed_urls:
            # Fallback: search the JSON string using regex
        ...
        # Resolve all episodes using the RSS XML
        rss_url = feed_urls[0]
        return parse_rss_feed(rss_url)

def resolve_url(url):
    """Dispatches parsing depending on URL type."""
    url = url.strip()
    if not url:
    ...
    if "podcasts.apple.com" in url:
        ...
        # Try direct RSS parsing
        return parse_rss_feed(url)
    else:
        # Attempt Apple Podcasts lookup if it is just a show ID
        if url.isdigit():
        ...
        raise ValueError("Unsupported URL. Please enter an Apple Podcasts link (show/episode) or a direct RSS XML Feed link.")

# ----------------- BACKGROUND DOWNLOADER -----------------

def download_worker():
    """Worker thread that executes downloads from the queue."""
    ...
    while True:
        ...
        with status_lock:
            # If cancellation was triggered while item was in queue
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=20) as response:
                ...
                with open(dest_path, "wb") as f:
                    while True:
                        ...
                        chunk = response.read(65536) # 64KB
                        if not chunk:
                            success = True
                            break
                        ...
                        # Update status every 0.5s or on completion
            ...
            if not success:
                # Cancelled or failed midway, delete incomplete file
        ...
        with status_lock:
            if download_status["cancel_flag"]:
                # If cancelled, discard output logs and just complete task
                download_queue.task_done()
                continue
        ...
        download_queue.task_done()
        time.sleep(1.0)  # Polite delay to prevent rate-limiting

def start_download_thread():
    """Starts the download worker thread if it is not already running."""
    global download_status
    with status_lock:
...
def cancel_all_downloads():
    """Cancels active download and flushes queue."""
    global download_status
    with status_lock:
...
class PodcastHTTPRequestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def end_headers(self):
        # Enable CORS for standard UI requests
    ...
    def serve_file(self, filename, content_type):
        try:
            # Files should exist in the same directory as app.py
        ...
        try:
            ...
            with status_lock:
                for ep in episodes:
                    ...
                    if date:
                        filename = f"[{date}] {safe_title}.mp3"
                    else:
            ...
            response = json.dumps({"status": "ok", "message": f"Queued {len(episodes)} episodes."}).encode('utf-8')
            self.send_response(200)
    ...
    def handle_api_status(self):
        with status_lock:
            # Deep-copy dictionary to return safely
            status_copy = {
    ...
    def handle_api_cancel(self):
        ...
        response = json.dumps({"status": "ok", "message": "All downloads cancelled."}).encode('utf-8')
        self.send_response(200)
        self.send_header("Content-Type", "application/json")
        self.send_header("Content-Length", str(len(response)))
        self.end_headers()
